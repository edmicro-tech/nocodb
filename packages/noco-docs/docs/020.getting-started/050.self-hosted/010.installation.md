...
tags: ['Open Source']
keywords : ['MicDB installation', 'MicDB docker installation', 'MicDB nodejs installation', 'MicDB quick try', 'MicDB prerequisites']
---
...
<details>
  ...
  #### Pull MicDB Image on Cloud Shell

  Since Cloud Run only supports images from Google Container Registry (GCR) or Artifact Registry, we need to pull MicDB image, tag it and push it in GCP using Cloud Shell. Here are some sample commands which you can execute in Cloud Shell.

  ```bash
  # pull latest MicDB image
...
<details>
  ...
  Configure Source Repository as `nocodb/nocodb`. Optionally you can pick release tag if you are interested in specific MicDB version.
  ...
  Pick a name for your MicDB application. This name will become part of URL subsequently
  ...
  - Select hosting plan for your MicDB application
...
<details>
  ...
  Go to [Templates](https://railway.app/templates), Search MicDB and click Deploy
