...
tags: ['Open Source']
keywords: ['MicDB upgrade', 'upgrade MicDB', 'upgrade nocodb']
---
...
Updating MicDB docker container is similar to updating [any other docker containers](https://www.whitesourcesoftware.com/free-developer-tools/blog/update-docker-images/).
...
# Previous docker run
#
terminal % docker run -d --name myMicDB \
...
# Find, stop and delete MicDB docker container
#
terminal % docker ps
CONTAINER ID   IMAGE                   COMMAND                  CREATED          STATUS                 PORTS                    NAMES
afdc8edd1005   nocodb/nocodb:0.111.0   "/usr/bin/dumb-init …"   18 seconds ago   Up 18 seconds          0.0.0.0:8080->8080/tcp   myMicDB
...
# Find and remove MicDB docker image
#
...
# Pull & run the latest MicDB image with same environment variables as before
#
terminal % docker run -d --name myMicDB \
-v "$(pwd)"/nocodb:/usr/app/data/ \
-p 8080:8080 \
