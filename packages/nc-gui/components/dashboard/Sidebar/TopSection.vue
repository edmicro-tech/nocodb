...
<template>
  ...
  <template v-else-if="!isSharedBase">
    <div class="xs:hidden flex flex-col p-1 gap-y-0.5 mt-0.25 mb-0.5 truncate">
      ...
      <NcButton v-if="isUIAllowed('workspaceSettings')" v-e="['c:team:settings']" type="text" size="small"
        class="nc-sidebar-top-button !xs:hidden" data-testid="nc-sidebar-team-settings-btn" :centered="false" :class="{
      ...
      <WorkspaceCreateProjectBtn v-model:is-open="isCreateProjectOpen" modal type="text"
      ...
      <WorkspaceCreateGroupBtn v-model:is-open="isCreateGroupOpen" modal type="text" @update:component="emit('update:component')"
        ...
        <div class="gap-x-2 flex flex-row w-full items-center !font-normal">
          ...
          <div class="flex">{{ $t('title.createGroup') }}</div>
        </div>
      </WorkspaceCreateGroupBtn>
    </div>
  </template>
</template>
