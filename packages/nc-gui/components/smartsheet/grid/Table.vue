...
<template>
            ...
            <thead v-show="hideHeader !== true" ref="tableHeadEl">
              <tr v-if="isViewColumnsLoading">
                <td v-for="(col, colIndex) of dummyColumnDataForLoading" :key="colIndex"
                  ...
                  <a-skeleton :active="true" :title="true" :paragraph="false" class="ml-2 -mt-2"
              ...
              <tr v-show="!isViewColumnsLoading" class="nc-grid-header">
                <th class="w-[85px] min-w-[85px]" data-testid="grid-id-column" @dblclick="() => { }">
                  <div class="w-full h-full flex pl-5 pr-1 items-center" data-testid="nc-check-all">
                    <template v-if="!readOnly">
                      ...
                      <div :class="{ hidden: !vSelectedAllRecords, flex: vSelectedAllRecords }"
                ...
                <th v-for="(col, index) in fields" :key="col.title" v-xc-ver-resize :data-col="col.id"
            ...
            <tbody v-if="headerOnly !== true" ref="tableBodyEl">
              <template v-if="showSkeleton">
                <tr v-for="(row, rowIndex) of dummyRowDataForLoading" :key="rowIndex">
                  <td v-for="(col, colIndex) of dummyColumnDataForLoading" :key="colIndex" class="border-b-1 border-r-1"
  ...
  'width': gridViewCols[columnObj.id]?.width || '200px',
}" :data-testid="`cell-${columnObj.title}-${rowIndex}`" :data-key="`data-key-${rowIndex}-${columnObj.id}`"
                      :data-col="columnObj.id" :data-title="columnObj.title" :data-row-index="rowIndex"
        ...
        <template #overlay>
          <NcMenu class="!rounded !py-0" @click="contextMenu = false">
            ...
            <NcMenuItem
              ...
              data-testid="nc-delete-row" @click="onRemoveRecordsClick()">
            ...
            <NcMenuItem v-if="contextMenuTarget" v-e="['a:row:copy']" class="nc-base-menu-item"
            ...
            <!-- {{ $t('general.clear') }} -->
            <NcMenuItem v-if="contextMenuTarget && hasEditPermission" class="nc-base-menu-item"
              data-testid="context-menu-item-paste" :disabled="isSystemColumn(fields[contextMenuTarget.col])"
              @click="paste">
            ...
            <NcMenuItem v-if="contextMenuTarget &&
              ...
              selectedRange.isSingleCell() &&
              (isLinksOrLTAR(fields[contextMenuTarget.col]) || !isVirtualCol(fields[contextMenuTarget.col]))
              " class="nc-base-menu-item" :disabled="isSystemColumn(fields[contextMenuTarget.col])"
              data-testid="context-menu-item-clear" @click="clearCell(contextMenuTarget)">
            ...
            <!-- Clear cell -->
            <NcMenuItem v-else-if="contextMenuTarget && hasEditPermission" class="nc-base-menu-item"
              :disabled="isSystemColumn(fields[contextMenuTarget.col])" data-testid="context-menu-item-clear"
    ...
    <LazySmartsheetPagination v-if="headerOnly !== true" :key="isMobileMode" v-model:pagination-data="paginationDataRef"
      :show-api-timing="!isGroupBy" align-count-on-right :align-left="isGroupBy" :change-page="changePage"
      ...
      <template #add-record>
        <div v-if="isAddingEmptyRowAllowed && !showSkeleton && !isPaginationLoading" class="flex ml-1">
          ...
          <a-dropdown-button v-else v-e="[isAddNewRecordGridMode ? 'c:row:add:grid:toggle' : 'c:row:add:form:toggle']"
            class="nc-grid-add-new-row" placement="top"
    ...
    <GeneralDeleteModal v-model:visible="isModalOpen" entity-name="Records" :on-delete="() => deleteSelectedRows()">
        ...
        <span>
          <div class="flex flex-row items-center py-2.25 px-2.5 bg-gray-50 rounded-lg text-gray-700 mb-4">
            ...
            <div class="capitalize text-ellipsis overflow-hidden select-none w-full pl-1.75"
              ...
              {{ data.filter((r) => r.rowMeta.selected).length }} records
            </div>
          </div>
        </span>
      </template>
